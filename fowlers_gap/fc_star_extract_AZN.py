...
This extracts fractional cover data for the star transects at Fowlers Gap
...
# Get imageList
imageDir = r'S:\fowlers_gap\imagery\landsat\seasonal_aridzone_fractional_cover'
imageList = []
idList = []
for d in np.unique(seasonalDates):
    ...
    if ym >201303:
        imageList.append(os.path.join(imageDir,
                                  'l8olre_nsw_m%s_dbia2_subset_AZN.tif'%d))  
    else:
        imageList.append(os.path.join(imageDir,
                                  'lztmre_nsw_m%s_dbia2_subset_AZN.tif'%d))         
    idList.append(ids[seasonalDates == d])


# Write the csvfile header
csvfile = r'seasonal_AZN_fc_extract.csv'
