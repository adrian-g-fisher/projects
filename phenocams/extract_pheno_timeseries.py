...
def main(baseDir, outDir):
    ...
    for siteDir in glob.glob(os.path.join(baseDir, '*')):
        ...
        csvFile = os.path.join(outDir, r'%s_timeseries.csv'%site)
        if os.path.exists(csvFile) is True:
            print("CSV file already exists for %s"%site)
            ...
            for i in imageList:
                ...
                maskDir = os.path.join(siteDir, 'mask')
                maskImage = os.path.join(maskDir, r'mask.JPG')
